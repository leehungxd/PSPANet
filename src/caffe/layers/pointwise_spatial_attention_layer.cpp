  ...
  for(int n = 0; n < num_; n++) {
    for(int h = 0; h < feature_H_; h++) {
      for(int w = 0; w < feature_W_; w++) {
        ...
        const int hstart = std::max(0, half_mask_H_ - h);
        const int hend = std::min(mask_H_, feature_H_ + half_mask_H_ - h);
        const int wstart = std::max(0, half_mask_W_ - w);
        const int wend = std::min(mask_W_, feature_W_ + half_mask_W_ - w);
